...
int getSortedMoves(int board[8][8], int n, int x, int y, int moves[8]) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (nx >= 0 && ny >= 0 && nx < n && ny < n && board[nx][ny] == -1) {
            ...
            moves[moveCount] = i;
            degree[moveCount] = countMoves(board, n, nx, ny);
            moveCount++;
        }
    }
...
int main() {
    ...
    if (knightTour(board, n, startx, starty, 1)) {
